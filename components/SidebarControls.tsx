...
const SidebarControls: React.FC<SidebarControlsProps> = ({ 
  ...
  const Slider = ({ label, value, min, max, step, onChange, precision = 1 }: any) => (
    <div className="mb-6 group">
      <div className="flex justify-between items-center mb-2 px-1">
        ...
        <span className="text-[10px] font-mono text-amber-500 font-bold bg-white/5 px-2 py-0.5 rounded-lg border border-white/5 shadow-inner">
  ...
  return (
    <div className="w-full h-full flex flex-col p-6 overflow-y-auto scrollbar-hide select-none bg-zinc-950/95 backdrop-blur-2xl border-l border-white/5">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-amber-500 font-black text-2xl italic tracking-tighter leading-none">BROW MAP PRO</h1>
          <p className="text-[8px] text-zinc-500 font-bold uppercase tracking-[0.4em] mt-1">Professional Android Visage</p>
        </div>
      ...
      <div className="flex gap-2 mb-8 overflow-x-auto scrollbar-hide pb-2">
        {(['visagism', 'position', 'shape', 'style', 'camera'] as ControlMode[]).map(mode => (
          ...
          >
            {mode === 'visagism' ? 'Ajustes' : mode === 'position' ? 'Posição' : mode === 'shape' ? 'Molde' : mode === 'style' ? 'Visual' : 'Câmera'}
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 space-y-2">
        {(activeMode === 'position' || activeMode === 'shape') && (
          <div className="flex p-1.5 bg-white/5 rounded-[22px] mb-8 border border-white/5">
            {['left', 'both', 'right'].map(side => (
              ...
              >
                {side === 'both' ? 'Simétrico' : side === 'left' ? 'Esquerda' : 'Direita'}
              </button>
            ))}
          </div>
        )}
        ...
        {activeMode === 'visagism' && (
          <div className="space-y-6">
            <div className="p-5 bg-white/5 rounded-[32px] space-y-5 border border-white/5 shadow-2xl">
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  <span className="text-[11px] text-zinc-100 font-bold uppercase tracking-wider">Mapeamento Geométrico</span>
                  <span className="text-[8px] text-zinc-500 font-medium">Ativar linhas de simetria facial</span>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  <span className="text-[11px] text-zinc-100 font-bold uppercase tracking-wider">Pontos de Controle</span>
                  <span className="text-[8px] text-zinc-500 font-medium">Exibir âncoras de edição direta</span>
                </div>
            ...
            <Slider label="Espessura dos Pontos" value={config.handleSize} min={2} max={16} step={0.5} onChange={(v: number) => updateValue('handleSize', v)} />
          </div>
        )}

        {activeMode === 'position' && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            <Slider label="Deslocamento Vertical" value={getDisplayValue('posY')} min={-500} max={500} step={1} onChange={(v: number) => updateValue('posY', v)} />
            <Slider label="Deslocamento Horizontal" value={getDisplayValue('posX')} min={-500} max={500} step={1} onChange={(v: number) => updateValue('posX', v)} />
            <Slider label="Nível de Zoom" value={getDisplayValue('scale')} min={0.2} max={4.0} step={0.01} precision={2} onChange={(v: number) => updateValue('scale', v)} />
            <Slider label="Inclinação / Rotação" value={getDisplayValue('rotation')} min={-90} max={90} step={0.5} onChange={(v: number) => updateValue('rotation', v)} />
          </div>
        )}
        ...
        {activeMode === 'shape' && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <Slider label="Espessura de Início" value={getDisplayValue('thickness')} min={1} max={50} step={0.5} onChange={(v: number) => updateValue('thickness', v)} />
          </div>
        )}

        {activeMode === 'style' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
            <Slider label="Visibilidade (Opacidade)" value={config.opacity} min={0.05} max={1} step={0.05} precision={2} onChange={(v: number) => setConfig(p => ({ ...p, opacity: v }))} />
            <div>
              <label className="text-[10px] font-black uppercase text-zinc-500 block mb-5 tracking-widest">Tom da Marcação</label>
              <div className="grid grid-cols-5 gap-4">
                {COLORS.map(c => (
                  <button key={c} onClick={() => setConfig(p => ({ ...p, color: c }))} className={`aspect-square rounded-2xl border-4 transition-all shadow-xl ${config.color === c ? 'border-white scale-110 shadow-amber-500/20' : 'border-white/5'}`} style={{ backgroundColor: c }} />
                ))}
              </div>
            </div>
            <button onClick={() => setConfig(p => ({ ...p, mirror: !p.mirror }))} className={`w-full py-5 rounded-3xl text-[10px] font-black uppercase border transition-all flex items-center justify-center gap-3 ${config.mirror ? 'bg-amber-500 text-black border-transparent shadow-xl' : 'bg-white/5 text-zinc-300 border-white/10'}`}>
              ...
              {config.mirror ? 'Espelhamento Ativo' : 'Câmera Direta'}
            </button>
          </div>
        )}
        ...
        {activeMode === 'camera' && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            <label className="text-[10px] font-black uppercase text-zinc-500 block mb-2 tracking-widest">Hardwares Disponíveis</label>
      ...
      <div className="mt-8 space-y-4 pb-safe">
        <button 
          onClick={() => {
            ...
            alert("Design Capturado com Sucesso!");
          }} 
        ...
        <button onClick={resetConfig} className="w-full py-3 text-zinc-600 hover:text-zinc-400 text-[10px] font-bold uppercase transition-colors">Reset de Fábrica</button>
      </div>
    </div>
  );
};
